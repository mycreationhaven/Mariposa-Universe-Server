...
export class AccessTokenService {
  ...
  private sig(v: string) {
    return createHmac('sha256', this.secret).update(v).digest('base64url');
  }
  issue(
    i: { userId: string; characterId: string; displayName: string },
  ...
  verify(token: string, now = Math.floor(Date.now() / 1000)) {
    ...
    const [h, p, s] = parts as [string, string, string],
      expected = this.sig(`${h}.${p}`),
      a = Buffer.from(s),
      b = Buffer.from(expected);
    if (a.length !== b.length || !timingSafeEqual(a, b)) throw new Error('Invalid access token');
    const c = JSON.parse(Buffer.from(p, 'base64url').toString()) as AccessClaims;
    if (!c.sub || !c.characterId || c.exp <= now)
      throw new Error('Expired or incomplete access token');
    return c;
  }
}
