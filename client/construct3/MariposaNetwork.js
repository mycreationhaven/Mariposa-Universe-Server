...
export class MariposaNetwork extends EventTarget{
  ...
  #capture(state){const now=performance.now();state.players.forEach((p,id)=>{const frames=this.remoteSnapshots.get(id)??[];frames.push({time:now,x:p.x,y:p.y,facing:p.facing,animation:p.animation});while(frames.length>3)frames.shift();this.remoteSnapshots.set(id,frames);});this.dispatchEvent(new Event('statechange'));}
}
