...
Initial tables are deliberately limited: users, characters, player sessions, economy accounts/transactions, seasonal sessions/claims, and audit logs. Player sessions store only peppered refresh-token hashes, expiration, and revocation state; plaintext refresh tokens exist only in the client response. Generate and apply migrations with `npm run db:generate` and `npm run db:migrate`.
