...
Simulation runs at 60 Hz; default desired network update rate is 20 Hz (Colyseus patches may be tuned to this during load testing). Remote clients render roughly 100 ms behind and interpolate between snapshots. Local prediction is intentionally a documented next step: record unacknowledged inputs, apply the shared movement rules, snap/reconcile to authoritative state, then replay inputs newer than `lastProcessedInputSequence`.
